...
static lv_obj_t *page_autoscan_create(lv_obj_t *parent, panel_arr_t *arr) {
    ...
    create_btn_group_item(&btn_group3, cont, 3, "Beeper", "On", "Boot", "Off", "", 4);
    ...
    btn_group_set_sel(&btn_group2, g_setting.autoscan.beeper);
    return page;
}
...
static void page_autoscan_on_click(uint8_t key, int sel) {
    if (sel == 0) {
        ...
        g_setting.autoscan.beeper = btn_group_get_sel(&btn_group3);
        ini_putl("autoscan", "beeper", g_setting.autoscan.beeper, SETTING_INI);
    }
}
